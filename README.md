...
> `TEANAPS`는 자연어 처리(NLP)와 텍스트 분석을 위한 `Python` 라이브러리 입니다. 텍스트 분석을 위해서는 `Python` 언어를 배운 후에도 다양한 자연어 처리 및 머신러닝 라이브러리를 활용할 줄 알아야합니다(e.g., KoNLPy, NLTK, Gensim, sklearn). 하지만 배워야하는 라이브러리가 적지않고 난이도도 높아 `Python` 언어에 충분히 익숙하지 않으신 분들은 접근하기가 쉽지 않습니다.  
